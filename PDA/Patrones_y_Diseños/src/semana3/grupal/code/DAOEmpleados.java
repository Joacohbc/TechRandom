...
public class DAOEmpleados {
	...
	public static boolean insert(Empleado empleado) {
		try {
			PreparedStatement statement = BDManager.getConnection().prepareStatement(INSERTL_EMPLEADO);
	...
	public static boolean update(Empleado empleado) {
		try {
			PreparedStatement statement = BDManager.getConnection().prepareStatement(UPDATE_EMPLEADO);
	...
	public static boolean delete(Empleado empleado) {
		try {
			PreparedStatement statement = BDManager.getConnection().prepareStatement(DELETE_EMPLEADO);
			statement.setString(1, empleado.getCedula());
	...
	public static LinkedList<Empleado> findAll() {
		...
		try {
			PreparedStatement statement = BDManager.getConnection().prepareStatement(ALL_EMPLEADOS);
	...
	public static Empleado find(String ciEmpleado) {
		try {
			PreparedStatement statement = BDManager.getConnection().prepareStatement(EMPLEADO_CI);
